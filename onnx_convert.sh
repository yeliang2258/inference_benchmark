...
do
  ...
  if [ "${model_file##*.}"x = "pdmodel"x ];then
    ...
    cd ..
    $PYTHON_CMD model_check.py --config_file conf/config.yaml --model_dir "$dir" --paddle_model_file "$model_file" --paddle_params_file "$params_file" --input_shape 3,224,224
  fi
done